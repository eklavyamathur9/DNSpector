...
This is the working roadmap for evolving DNSpector from a solid single-signal detector into a portfolio-grade DNS security tool. Background on *why* each item matters (the DNS/security theory) lives in [DOCUMENTATION.md](DOCUMENTATION.md) — this file is the ordered, checklist-driven execution plan.
...
- [x] (Not originally scoped, done anyway) Module split into the `dnspector/` package — see Phase 1's now-checked-off item above
...
*Also landed in this phase (not originally scoped, but the natural trigger point): split the ~630-line single file into the `dnspector/` package (8 modules), deferred from Phase 1 as planned there. `dnspector.py` is now a thin backward-compatible shim; `python -m dnspector` also works. Test suite reorganized to mirror the package (`tests/test_dns_parsing.py`, `test_detection.py`, `test_threat_intel.py`, `test_config.py`, `test_cli.py`, `test_analysis.py`) and grew from 64 to 95 cases, including a real synthetic-pcap end-to-end test in `test_analysis.py` (closing the "still open" test-coverage gap noted in Phase 1/2).*
...
- [x] Webhook alerting (Slack/Discord/email) on high-severity remarks — new `alerting.py`: `classify_severity()` + `WebhookAlerter`; `--enable-alerts`, `--webhook-url`/`DNSPECTOR_WEBHOOK_URL`, `--alert-min-severity` (info/medium/high/critical); works in both batch and live mode, sharing the same alerter
...
- [x] Tag a few small, well-scoped items as "good first issue" — **5 real issues filed** on GitHub (required enabling Issues on the repo first, which had been off): [#1 mypy in CI](https://github.com/eklavyamathur9/DNSpector/issues/1), [#2 sliding-window burst backport](https://github.com/eklavyamathur9/DNSpector/issues/2), [#3 alert de-duplication](https://github.com/eklavyamathur9/DNSpector/issues/3), [#4 typosquatting detection](https://github.com/eklavyamathur9/DNSpector/issues/4), [#5 live dashboard](https://github.com/eklavyamathur9/DNSpector/issues/5)
- [x] `CHANGELOG.md` — Keep a Changelog format, one entry per landed phase (0.1.0 → 0.6.0), reconstructed from the git history
- [x] Package for PyPI (`pyproject.toml` build metadata) — verified end-to-end: built and installed into a throwaway venv, confirmed `dnspector.__version__` resolves correctly (dynamically sourced from `_version.py`, not duplicated) and the `dnspector` console-script entry point works. Also added `.github/workflows/publish.yml` (PyPI Trusted Publishing via OIDC, no stored secret) so a real release can ship with one click once set up. **Not yet actually published to PyPI** - claiming the package name is a real, external, one-way action requiring a one-time manual step on pypi.org only the account owner can do (documented in `CONTRIBUTING.md`), deliberately left for the user rather than attempted without their credentials.
- [x] Enable GitHub Discussions for design conversations separate from issues — enabled via `gh repo edit --enable-discussions`.

*Landed 2026-08-20. At the time, checked PyPI availability for both spellings of the then-current name (`dns-analyzer`/`dns-analyser`) before setting up packaging - both were unclaimed. The project has since been renamed to **DNSpector** (see Phase 7); `dnspector` was separately confirmed unclaimed on PyPI as part of that decision, and the packaging metadata now targets that name instead. Creating the 5 "good first issue" tickets required first discovering (and enabling) that the repository had Issues disabled entirely - not just missing content. Semantic-versioning git tags for 0.1.0–0.6.0 were not created - `CHANGELOG.md` records the versions, but backdating tags onto existing commits is a bit unusual and was left out; new tags can start from whatever version ships next (see `CONTRIBUTING.md`'s release walkthrough).*

---

## Phase 7 — Naming / Rebrand ✅ *done*

**Why:** deferred until scope was clearer post-Phase 2/3 — renaming any earlier would have been premature. Landed once Phases 1–6 gave the project real, broad scope (multi-signal detection, threat intel, live monitoring, SIEM export) that a name like "DNS Analyzer" undersold.

**Decision: DNSpector.** DNS + Inspector — chosen over the other candidates specifically because the roadmap spans general DNS threat-hunting (entropy, baselining, bursts, threat intel, live monitoring, SIEM export) rather than one narrow technique, which ruled out the more technique-specific options (TunnelTrace, DGAWatch). Runner-up was Sentry53.

| Name | Rationale | Status |
|---|---|---|
| **DNSpector** | DNS + Inspector — broad, fits both anomaly detection and general forensics scope | **Chosen** |
| Sentry53 | References port 53 + "sentry" (watch/guard) — memorable, security-forward | Runner-up |
| DNSentinel | DNS + Sentinel — straightforward, communicates active monitoring | Considered |
| DNSleuth | DNS + sleuth — catchy, leans into the investigative/forensic angle | Considered |
| TunnelTrace | Leans specifically into tunneling/exfiltration detection — narrower scope | Ruled out (too narrow) |
| DGAWatch | Leans specifically into DGA detection — narrower scope | Ruled out (too narrow) |

**What the rename touched:** the `dns_analyzer/` package → `dnspector/` (every internal import updated); the root-level entry shim `Dns_Analyser.py` → `dnspector.py`; the GitHub repository `Dns-Analyser` → `DNSpector` (via `gh repo rename`, plus a repo description set for the first time); the local git remote URL updated to match; PyPI packaging metadata (`pyproject.toml`'s `name`, the `dnspector` console-script entry point, all project URLs); the `DNS_ANALYZER_WEBHOOK_URL` environment variable → `DNSPECTOR_WEBHOOK_URL`; the CEF vendor/product identifiers in `syslog_forwarder.py` (`DNSAnalyzer`/`dns-analyzer` → `DNSpector`/`dnspector`); the PDF report's title text; every reference across `README.md`, `DOCUMENTATION.md`, `CONTRIBUTING.md`, `SECURITY.md`, `CHANGELOG.md`, `.github/` templates and workflows; and the demo screenshot/terminal output in the README, regenerated end-to-end under the new package rather than hand-edited.

*Landed 2026-08-21. Confirmed `dnspector` unclaimed on PyPI before deciding (all six candidates were, in fact - checked via the PyPI JSON API). GitHub's repo-rename automatically keeps the old `Dns-Analyser` URL working as a redirect, but the local git remote was still repointed to the canonical new URL rather than relying on that indefinitely. Full test suite (190 cases) re-verified green after the rename, and both entry points (`python dnspector.py`, `python -m dnspector`) re-verified working. The project's local working directory name was deliberately left unrenamed (a filesystem detail, not part of the public project identity, and renaming it mid-session risked disrupting the current session's working directory).*
